...
function initMap() {
    try {
        ...
        document.getElementById('toggle-user-markers').addEventListener('change', toggleUserMarkers);
        document.getElementById('toggle-heritage-markers').addEventListener('change', toggleHeritageMarkers);
    } catch (error) {
...
function addHeritageMarker(position, title, link) {
    ...
    const infoWindow = new google.maps.InfoWindow({
        content: `<div><strong>${title}</strong><br><a href="${link}" target="_blank">Wikipedia</a><br><a href="https://www.google.com/maps/dir/?api=1&destination=${position.lat},${position.lng}" target="_blank">Wyznacz trasę</a></div>`
    });
    ...
    try {
        ...
        let contentString = `
            <div>
                ...
                <a href="https://www.google.com/maps/dir/?api=1&destination=${location.lat},${location.lng}" target="_blank">Wyznacz trasę</a>
        `;
...
function saveMarker(markerData) {
    fetch('/add_marker', {
      ...
      .then(data => {
          if (data.success) {
              const marker = userMarkers.find(m => m.id === markerData.id);
              if (marker) {
    ...
    fetch(`/delete_marker/${markerId}`, {
      ...
      .then(data => {
          if (data.success) {
              ...
              if (marker) {
                  marker.setMap(null);
                  userMarkers = userMarkers.filter(m => m.id !== markerId);
              }
              document.getElementById('marker-form').style.display = 'none';
          } else {
...
function toggleUserMarkers() {
    const show = document.getElementById('toggle-user-markers').checked;
    userMarkers.forEach(marker => marker.setMap(show ? map : null));
}

function toggleHeritageMarkers() {
    const show = document.getElementById('toggle-heritage-markers').checked;
    heritageMarkers.forEach(marker => marker.setMap(show ? map : null));
}
